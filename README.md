### **Resultados da Entrevista de Carreira em Tecnologia e Plano de Estudos**

Este repositório contém um exercício desenvolvido no Bootcamp CAIXA – Inteligência Artificial na Prática, promovido pela DIO. Os prompts fornecidos para a atividade foram disponibilizados pela própria DIO, enquanto a análise da entrevista foi realizada pelo Copilot IA.

### AGENT 1

### Perguntas e Respostas

**O que mais te atrai em tecnologia?**  
R: Criar produtos.

**Você já tem experiência na área de tecnologia ou está começando do zero?**  
R: Tenho certa experiência na área de tecnologia como operadora de mainframe, mas não tenho experiência em criar produtos.

**Quantas horas por semana você consegue dedicar aos estudos?**  
R: 12 horas.

**No seu dia a dia, você prefere lidar mais com pessoas, dados ou código?**  
R: Código.

**Qual é seu objetivo principal?**  
R: Fazer transição de carreira.

**Quais assuntos ou tecnologias mais despertam seu interesse?**  
R: Inteligência artificial e desenvolvimento de programas.

**Você tem alguma experiência prévia (mesmo que não seja em tech) que gostaria de aproveitar nessa nova jornada?**  
R: Além de operadora de mainframe, já fui vendedora.

---

### Análise e Sugestão de Carreiras

Com base nas suas respostas, identifiquei 3 carreiras muito promissoras:

### 1º LUGAR: **Desenvolvedora de Software / Programadora – 18/20**

### Por que combina com você:
- Você gosta de criar produtos e prefere lidar com código.  
- Já tem experiência em ambientes críticos (mainframe), o que mostra disciplina e atenção a detalhes.  
- A transição aproveita sua base técnica e sua curiosidade por desenvolvimento.

### O que esperar:
**Vantagens:**
- Alta demanda em praticamente todas as regiões.  
- Possibilidade de atuar em diversas áreas (web, mobile, sistemas corporativos).

**Desafios:**
- Ramp-up exige prática constante em lógica e linguagens modernas.  
- Necessário se atualizar em frameworks e metodologias ágeis.

**Mercado:**  
Desenvolvedores são requisitados em empresas de todos os portes. A demanda varia por região e experiência, mas é uma das carreiras mais sólidas em tecnologia.

---

### 2º LUGAR: **Cientista de Dados / Engenheira de IA – 16/20**

### Por que combina com você:
- Você demonstrou interesse em inteligência artificial.  
- Sua experiência com mainframe mostra que já entende sistemas complexos.  
- O perfil analítico e a curiosidade por criar soluções inteligentes são fortes pontos.

### O que esperar:
**Vantagens:**
- Área em crescimento acelerado.  
- Projetos inovadores com impacto direto em produtos e negócios.

**Desafios:**
- Exige aprendizado em estatística, matemática e ferramentas de machine learning.  
- Ramp-up pode ser mais longo que em desenvolvimento tradicional.

**Mercado:**  
IA e dados são áreas estratégicas em empresas de tecnologia, finanças, saúde e varejo. A demanda é alta, mas varia conforme especialização e região.

---

### 3º LUGAR: **Analista de Produto / Product Manager Tech – 14/20**

### Por que combina com você:
- Você tem experiência como vendedora, o que ajuda a entender clientes e mercado.  
- Gosta de criar produtos e pode unir visão de negócio com tecnologia.  
- Ideal para quem quer transição aproveitando habilidades de comunicação e organização.

### O que esperar:
**Vantagens:**
- Papel estratégico, conecta tecnologia e negócio.  
- Aproveita sua experiência prévia fora da área técnica.

**Desafios:**
- Menos foco em código, mais em gestão e comunicação.  
- Exige aprendizado em metodologias ágeis e ferramentas de produto.

**Mercado:**  
Product Managers são cada vez mais valorizados em startups e grandes empresas, mas a demanda varia por maturidade do mercado local.

---

### AGENT 2

### Plano de Estudos

### Visão do Dia a Dia
- Coletar e organizar dados  
- Explorar e analisar padrões  
- Criar modelos de machine learning  
- Testar e validar modelos  
- Comunicar resultados para equipes  

---

### Mapa de Skills

**Core Skills:** Python, Estatística, Machine Learning  
**Nice-to-Have:** Visualização de dados, Bancos de dados  
**Ferramentas:** Pandas, NumPy, Scikit-learn, Jupyter Notebook, GitHub

---

### Roadmap de 90 Dias

**Mês 1:** Fundamentos (Python, Pandas, Estatística)  
**Mês 2:** Prática (datasets reais, métricas, algoritmos avançados)  
**Mês 3:** Portfólio e preparação (projeto completo, GitHub, simulação de entrevistas)

---

### Projeto de Portfólio

**Tema:** Previsão de Vendas com Machine Learning  
**Entregáveis:** Notebook, relatório de métricas, dashboard simples  
**Critérios:** Modelo validado, código organizado, repositório público

---

### Roteiro de Entrevistas
- Explique um projeto de dados  
- Diferença entre regressão e classificação  
- Como lidar com dados faltantes  
- Métricas de avaliação de classificação  
- Explicar modelo complexo para não técnicos  

---

### Trilha DIO Recomendada

**Bootcamp:** Ciência de Dados com Python  
**Por que:** Conecta fundamentos de Python, estatística e ML com prática em projetos reais  
**Próximos passos:** acessar dio.me, buscar a trilha, inscrever-se e seguir junto ao roadmap

---

### Autor

**Rhayanne Matos**  
Desenvolvido como parte do Bootcamp CAIXA – Inteligência Artificial na Prática (DIO).
